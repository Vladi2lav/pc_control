...
3. Как работает переключение режимов работы (Portable vs Installed):
   - Портативный режим (Portable):
     ...
     Lumen.exe. В этом режиме программа хранит настройки локально в подкаталоге
     `data/settings/`, а пользовательские модули (плагины) всегда сохраняются в
     постоянных стандартных директориях системы (чтобы они выживали при удалении
     портативной папки).
...
with open(portable_dir / "portable.txt", "w", encoding="utf-8") as f:
    f.write("Этот файл переводит Lumen в портативный режим.\nНастройки будут сохраняться локально в папке 'data/settings/'.\nМодули по-прежнему сохраняются в стандартной папке системы, чтобы не потеряться при удалении приложения.\n")
